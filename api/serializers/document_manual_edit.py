...
# class DocumentManualEditFinishSerializer là serializer định nghĩa dữ liệu vào/ra (DocumentManualEditFinish).
...
# class DocumentManualEditSessionSerializer là serializer định nghĩa dữ liệu vào/ra (DocumentManualEditSession).
# vd: serializer.data -> JSON cho frontend; is_valid() kiểm tra dữ liệu gửi lên.
class DocumentManualEditSessionSerializer(serializers.ModelSerializer):
    ...
    # def get_lock_message để lấy lock message (trong serializer).
    # vd: nhận điều kiện -> trả về dữ liệu phù hợp.

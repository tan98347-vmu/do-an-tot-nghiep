...
AI Document Manager là nền tảng web hỗ trợ doanh nghiệp quản lý, soạn thảo, chia sẻ, phê duyệt và ký số văn bản trên một hệ thống tập trung. Dự án kết hợp Flutter Web, Django REST Framework, PostgreSQL, AI/Ollama, Collabora và Microsoft Word worker để tạo thành một quy trình xử lý tài liệu đầy đủ từ tạo mẫu đến phát hành văn bản.
...
- Tích hợp Microsoft Word add-in và worker agent cho các tác vụ chỉnh sửa Word nâng cao.
- Có cấu hình triển khai với nginx, Collabora/CODE, backup công ty và các command quản trị dữ liệu.
...
Người dùng
  ...
  -> PostgreSQL
  -> AI/Ollama, OCR, RAG
  -> Collabora/CODE hoặc Word worker khi cần xử lý DOCX
```
...
- `word_ai/`, `word_addin/`, `word_worker_agent/`: tích hợp Microsoft Word và worker xử lý native tool.
- `my_tennis_club/`: cấu hình Django, route gốc, ASGI/WSGI và management commands.
- `deploy/`: cấu hình triển khai local stack, nginx, Cloudflare tunnel và Word worker host.
...
- Microsoft Word trên máy worker nếu dùng Word native worker.
...
- Các cấu hình signing, remote HSM, LibreOffice và Word worker nếu dùng trong production.
...
Hệ thống hỗ trợ hai hướng xử lý tài liệu nâng cao:

- Collabora/CODE cho chỉnh sửa DOCX qua trình duyệt và WOPI endpoint.
- Word worker agent cho các tác vụ cần Microsoft Word native tool.

Các biến `MANUAL_EDIT_PROVIDER`, `COLLABORA_PUBLIC_URL`, `MANUAL_EDIT_WOPI_SRC_BASE_URL`, cấu hình nginx và các script trong `deploy/` cần được điều chỉnh theo môi trường local hoặc production.
...
Tùy phạm vi thay đổi, nên chạy thêm các test theo module như `accounts`, `documents`, `signing`, `sharing`, `word_ai` và các test Flutter liên quan màn hình vừa sửa.
...
- Nếu cần tài liệu vận hành, đặt trong `docs/` hoặc `deploy/` với nội dung rõ mục đích sử dụng.
